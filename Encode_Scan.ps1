#config
    ...
    cd $rootencode # set directory of root folder for monitored videos
    ...
    $DeleteCSV = $True #Set this value to true if you wish to delete contents.csv after encoding compelte
#Functions
...
#Begin scanning files
    ...
    If ($alldirectories -eq $False){
        For ($i = 1; $i -le 3; $i++) {
            if ($i -eq 1) {$gc = "$rootencode\TV\"}Elseif ($i -eq 2) {$gc = "$rootencode\Movies\"}Elseif ($i -eq 3) {$gc = "$rootencode\Anime\"} 
            Get-ChildItem -Path $gc -Recurse -Include "*" | % {$_.FullName} | Write-Output | Add-Content $rootencode\contents.txt

        }
    }Else{Get-ChildItem -Path $rootencode -Recurse -Include "*" | % {$_.FullName} | Write-Output | Add-Content $rootencode\contents.txt}
